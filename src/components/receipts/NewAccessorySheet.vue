...
/**
 * Create a brand-new accessory / keypad phone for a receipt — its CATALOG
 * identity only (name, IMEI for phones, image, note) plus how many arrived.
 * Purchase price and sale price belong to the intake itself, so they're entered
 * on the receipt row (prixod), not here. This does NOT hit the API — the backend
 * persists the item when the receipt is saved.
 */
...
const form = reactive<{
  ...
  note: string
}>({ name: '', imei: '', quantity: '1', imageUrl: null, note: '' })
